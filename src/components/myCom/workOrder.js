...
import { Input ,Button,Form,Icon,Radio,Collapse,Modal,Timeline   } from 'antd';
...
class workOrder extends React.Component{
  ...
  render(){
    ...
    const { executeInfo } = this.props.EPatrol;    //工单详情
    //巡检结果
    ...
    return(
      <div className={styles.workOrderMain}>
        <Form>
          <FormItem
            colon={false}
            label={<div style={{marginRight:5, marginTop:5}}><span id="titIcon" style={titIconStyle}></span>标题</div>}
            {...formItemLayout}
          ...
          <Collapse bordered={false} defaultActiveKey={['5']}>
                ...
                <Panel header="巡检信息" key="1"  style={customPanelStyle} className={styles.borderBottom}>
                  ...
                  >
                    {getFieldDecorator('checkItem', {    //prodName==>定义唯一ID，也是key
                      initialValue:executeInfo.checkLog ? executeInfo.checkLog.nameModel :null,   //定义初始值  executeInfo.checkLog.checkItem
                    })(
                      <Input
                        type="text"
                        disabled={true}
                  ...
                  >
                    ...
                    <div className={styles.workOrderDiv}>
                      <div style={{marginTop:10}}>
                        {
                          executeInfo && executeInfo.checkLog && executeInfo.checkLog.ckImg1 ? <img onClick={BigckImg5} src={executeInfo.checkLog.ckImg1} className={styles.workOrderImg} /> : null
                        }
                        {
                          executeInfo && executeInfo.checkLog && executeInfo.checkLog.ckImg2? <img onClick={BigckImg6} src={executeInfo.checkLog.ckImg2} className={styles.workOrderImg} /> : null
                        }
                        {
                          executeInfo && executeInfo.checkLog &&  executeInfo.checkLog.ckImg3 ? <img onClick={BigckImg7} src={executeInfo.checkLog.ckImg3} className={styles.workOrderImg} /> : null
                        }
                        {
                          executeInfo && executeInfo.checkLog &&  executeInfo.checkLog.ckImg4 ? <img onClick={BigckImg8} src={executeInfo.checkLog.ckImg4} className={styles.workOrderImg} /> : null
                        }
                      </div>
                  ...
                  <TextArea  rows={3} disabled={true} className={styles.borderNone} style={{fontSize:13,color:'#555'}} />
                  )}
                  </FormItem>
                </Panel>
              ):null
                ...
                <Panel header="工单信息" key="2" style={customPanelStyle} className={styles.borderBottom}>
                  ...
                  <div className={styles.workOrderDiv2}>
                    <div>
                      <span>
                        {
                          taskImgObj[0] ||audioSrcObj ||videoSrcObj ? <label style={{marginLeft:-45}}>图/音/视:</label> : null
                        }
                      </span>
                      {
                    ...
                    <div className={styles.workOrderHidden}>
                      {
                        audioSrcObj && audioSrcObj!=null && audioSrcObj!="" ? <audio src={audioSrcObj} controls="controls"></audio> : null
                      }
                    </div>
                    <div className={styles.workOrderHidden}>
                      {
                        videoSrcObj && videoSrcObj!=null && videoSrcObj!="" ? <video src={videoSrcObj} poster={videoSrcPng} width="260px" height="124px" controls="controls"></video> : null
                      }
                    </div>
                  </div>
            ...
            <Panel header="维修报告" key="4" style={customPanelStyle} className={styles.borderBottom}>
              <FormItem
                label="维修报告"
                {...formItemLayout}
                className={styles.workOrderTit}
              ...
              <div>
                ...
                <span>
                  {
                    executeInfo.taskLog && executeInfo.taskLog.img1 || tasklogAudioObj !== 'null'||tasklogVideoObj !== 'null' ||tasklogAudioObj ||tasklogVideoObj ? <label style={{marginLeft:15}}>图/音/视:</label> : null
                  }
                </span>
                {
                  executeInfo.taskLog && executeInfo.taskLog.img1 && executeInfo.taskLog.img1 !=null && executeInfo.taskLog.img1 !="" ? <img onClick={BigckImg9} src={executeInfo.taskLog.img1} className={styles.workOrderImg} /> : null
                }
                {
                  executeInfo.taskLog && executeInfo.taskLog.img2 && executeInfo.taskLog.img2 !=null && executeInfo.taskLog.img2 !="" ? <img onClick={BigckImg10} src={executeInfo.taskLog.img2} className={styles.workOrderImg} /> : null
                }
                {
                  executeInfo.taskLog && executeInfo.taskLog.img3 && executeInfo.taskLog.img3 !=null && executeInfo.taskLog.img3 !="" ? <img onClick={BigckImg11} src={executeInfo.taskLog.img3} className={styles.workOrderImg} /> : null
                }
                {
                  executeInfo.taskLog && executeInfo.taskLog.img4 && executeInfo.taskLog.img4 !=null && executeInfo.taskLog.img4 !="" ? <img onClick={BigckImg12} src={executeInfo.taskLog.img4} className={styles.workOrderImg} /> : null
                }
              </div>
              <div className={styles.taskAudio}>
                {
                  tasklogAudioObj && tasklogAudioObj!== null &&tasklogAudioObj !== 'null'   && tasklogAudioObj!== "" && tasklogAudioObj!== undefined ?  <audio src={tasklogAudioObj}  controls="controls"></audio> : null
                }
              </div>
              <div className={styles.taskAudio}>
                {
                  tasklogVideoObj && tasklogVideoObj!= null &&tasklogVideoObj !== 'null'   && tasklogVideoObj!= ""  && tasklogVideoObj!= undefined ? <video src={tasklogVideoObj} poster={tasklogVideoPng} width="251px" height="124px" controls="controls"></video> :  null
                }
              </div>
            </Panel>
            <Panel header="工单追踪" key="5" id="5" style={customPanelStyle} className={styles.borderBottom}>
              {
                timelineInfo != null && timelineInfo != undefined && timelineInfo != "" ?
                  <Timeline>
                    {timelineInfo.map((timeline) => {
                      return (<Timeline.Item key={ timeline.logId}>
                        ...
                        {
                          timeline && timeline.curTaskLink == "工单派单" ? timeline.subInfo.map((subInfo)=> {
                            return (<div key={subInfo.taskId}>
                              <br/>
                              <p><span>工单名称:</span>{subInfo.taskTitle}</p>
                              <p>
                                {subInfo.taskStatus != "1" ?
                                  <div>工单状态:{subInfo.taskStatusCn}<p><span>当前处理人:{subInfo.executor}</span></p></div> :
                                  <span>已完成</span>}
                              </p>
                            </div> )
                          }) : null
                  ...
                  : <label style={{marginLeft:15}}>暂无执行记录</label>
              }
            </Panel>
          </Collapse>
          <br/>
